...
module spi (
    ...
    reg  [ 7:0] shift_reg;  // Receive shift register for active transfer
    reg  [ 7:0] tx_shift_reg;  // Transmit shift register for active transfer
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) read_completed <= 1'b0;
        else read_completed <= read_completing;
    end
            ...
            case (state)
                ...
                STATE_TRANSFER: begin
                    if (mstr) begin
                        ...
                        if (do_sample) begin
                            // Sample MISO
                            if (dord) shift_reg <= {miso, shift_reg[7:1]};  // LSB first
                            else shift_reg <= {shift_reg[6:0], miso};  // MSB first
                            sample_cnt <= sample_cnt + 1'b1;
                        end

                        if (do_shift) begin
                            bit_cnt <= bit_cnt + 1'b1;
                            if (!(cpha && first_edge)) begin
                                if (dord) tx_shift_reg <= {1'b0, tx_shift_reg[7:1]};
                                else tx_shift_reg <= {tx_shift_reg[6:0], 1'b0};
                            end
                            first_edge <= 1'b0;
                        end

                        // Complete on trailing edge for CPHA=0, sample edge for CPHA=1
                        if ((cpha == 1'b0 && bit_cnt == 4'd7 && do_shift) ||
                            (cpha == 1'b1 && sample_cnt == 3'd7 && do_sample)) begin
                        ...
                        if (!slave_selected) begin
                            ...
                            if (slave_sample) begin
                                ...
                                if (dord) shift_reg <= {mosi_in, shift_reg[7:1]};
                                else shift_reg <= {shift_reg[6:0], mosi_in};
                                sample_cnt <= sample_cnt + 1'b1;
                            end
                            ...
                            // Complete on trailing edge for CPHA=0, sample edge for CPHA=1
                            if ((cpha == 1'b0 && bit_cnt == 4'd7 && slave_shift) ||
                                (cpha == 1'b1 && sample_cnt == 3'd7 && slave_sample)) begin
                                state <= STATE_COMPLETE;
                            end
                        end
                    end
    ...
    wire tx_bit = dord ? tx_shift_reg[0] : tx_shift_reg[7];
